...
ASR_THRESHOLD = 0.28  # cosine similarity below this = identity not recognised
...
def load_arcface(weights_path, device):
...
def arcface_embed(model, x):
    """x: [B, 3, 112, 112] in [0, 1]. Returns L2-normalised embeddings [B, 512]."""
    return F.normalize(model(NORMALIZE(x)), p=2, dim=-1)


def pgd_untargeted_batch(model, x_orig, epsilon, steps, alpha):
    """
    Batched untargeted PGD: minimises cosine similarity between clean and cloaked embeddings.
    Operates in fp32. x_orig: [B, 3, 112, 112] on device, values in [0, 1].
    Returns x_adv on CPU.
    """
    x_orig = x_orig.detach()
    with torch.no_grad():
        e_orig = arcface_embed(model, x_orig)

    # Random start within L-inf ball (clipped to valid pixel range)
    ...
    for _ in range(steps):
        ...
        e_adv = arcface_embed(model, x_adv)
        # Minimise cosine similarity: negate so gradient ascent on -loss = descent on cos_sim
        loss = -F.cosine_similarity(e_adv, e_orig.detach(), dim=-1).sum()
        loss.backward()
...
def parse_args():
    p = argparse.ArgumentParser(description="Batched untargeted PGD against ArcFace.")
    p.add_argument("--manifest", type=Path, default=Path("data/lfw/manifest.csv"))
    p.add_argument("--arcface_weights", type=Path,
...
def main():
    ...
    results_csv = args.output_dir / f"pgd_untargeted_arcface_{tag}_results.csv"
    metrics_csv = args.output_dir / f"pgd_untargeted_arcface_{tag}_metrics.csv"
    ...
    manifest["_path"] = manifest["arcface_path"].apply(fix_path)
    print(f"Manifest sampled: {len(manifest)} images")

    # Resume support: skip rows already in the checkpoint CSV
    done_paths = set()
    existing_rows = []
    ...
    transform = T.Compose([T.Resize((ARCFACE_SIZE, ARCFACE_SIZE)), T.ToTensor()])

    print("Loading ArcFace (iresnet100)...")
    model = load_arcface(args.arcface_weights, device)
    ...
    for batch_df in tqdm(batches, desc=f"PGD batches eps={args.epsilon}"):
        ...
        with torch.no_grad():
            e_orig = arcface_embed(model, x_orig)
            e_adv = arcface_embed(model, x_adv.to(device))
            cos_sims = F.cosine_similarity(e_orig, e_adv, dim=-1).cpu().numpy()

        for i, (row, cos_sim) in enumerate(zip(meta, cos_sims)):
            ...
            asr = int(cos_sim < ASR_THRESHOLD)

            if args.save_images and images_dir:
                fname = Path(str(row["_path"])).stem + "_cloaked.png"
    ...
    print(f"  cos_sim  : {mean_cos:.4f}  (threshold={ASR_THRESHOLD})")
    print(f"{'='*52}")
